function result = test_all_method(data, split, trK, teK, para)
...
switch (para.method)
    ...
    case 'alter-pSVM-pin'
        ...
        while(para.C ~= C)
            para.C = min(para.C*1.5,C);
            model = alternating_svm_pin_dual_with_slack(data, trK, split.train_bag_idx, split.train_bag_prop, para);
            para.init_y = model.y;
        end
...
%
result.train_acc = length(find(result.predicted_train_label - split.train_label == 0))/ length(model.y) * 100;
result.test_acc = length(find(result.predicted_test_label - split.test_label==0))/length(predict_label) * 100;
%
result.train_bag_error = get_bag_err(result.predicted_train_label, split.train_bag_idx, split.train_bag_prop) * 100;
result.test_bag_error = get_bag_err(result.predicted_test_label, split.test_bag_idx, split.test_bag_prop) * 100;
%
result.model = model;
