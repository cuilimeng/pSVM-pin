function [model] = alternating_svm_pin_dual_with_slack(data, K, Bag_idx, Bag_prop, para)
...
while (~ifconverge && iter<=para.maxiter)
    model_pre = model; model.tau = para.tau;
    model = optimize_w(data, K, model, para);
    if isempty(model)
...
%author:clm
%date:20150910
%update:change hinge loss to pinball loss
xi_n = max(1 - y_n.*f, zeros(length(f),1));
xi_p = max(1 - y_p.*f, zeros(length(f),1));
% xi_n = max(1 - y_n.*f, -model.tau * (1 - y_n.*f));
% xi_p = max(1 - y_p.*f, -model.tau * (1 - y_p.*f));
end
...
%author:clm
%date:2015.09.09
%update:change regular SVM to pinSVM
...
% end
function [ model ] = optimize_w(data, K, model, para)
    if (para.SVR == 0)
        [alphas, offset] = pin_SVM_path(data, model.y, 0.0001 * ones(length(model.y), 1), para.tau);
    else
        model_svm = regular_svr_wrapper(K, model.y, para);
    end
    model.alp = model.y.*alphas(:,2); % update the model
    model.b = offset(2);
    model.y_real = K * (model.alp) + model.b; % predicted y
end
...
function [objective, obj_1, obj_2] = compute_obj(K, model, para)
    if para.SVR == 0
        ...
        xi = max(1 - model.y.*model.y_real, - para.tau * (1 - model.y.*model.y_real));
    else
        xi = 0.5*(1 - model.y.*model.y_real).^2;
    end
    %author:clm
    %date:20150909
    %update:change regularization item
    %obj_1 = para.C * sum(xi) + 0.5* quad_form(model.alp, K(model.support_v, model.support_v));
    obj_1 = para.C * sum(xi) + 0.5* quad_form(model.alp, K);
